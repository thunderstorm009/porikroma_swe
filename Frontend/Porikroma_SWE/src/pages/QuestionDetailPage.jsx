...
import { ArrowLeft, Bell, Bookmark, Heart, MessageCircle, Send, Sparkles, UserRound } from 'lucide-react';
...
function Person({ user, compact = false }) { return <div className="forum-person"><span className={`avatar avatar-${user?.color || 'teal'}`}>{user?.initials || '?'}</span><span><strong>{user?.name || 'Traveler'}</strong><small>{user?.role || 'Traveler'}{!compact && ` · ${user?.contributions || 0} contributions`}</small></span></div>; }

export default function QuestionDetailPage({ questionId, onNavigate, theme, onToggleTheme }) {
  ...
  useEffect(() => { 
    let active = true; 
    // Only show full loading state if we don't have the bundle or switching questions
    if (!bundle || bundle.question?.id !== questionId) {
      setLoading(true); 
    }
    forumService.getQuestionBundle(questionId).then((loaded) => { 
      if (!active) return; 
      setBundle(loaded); 
      setLoading(false);
      // Fetch similar questions in the background
      if (loaded?.question) {
        forumService.getSimilarQuestions(loaded.question).then(similarItems => {
          if (active) setSimilar(similarItems);
        }).catch(() => {});
      }
  ...
  if (loading) return <TravelShell onNavigate={onNavigate} theme={theme} onToggleTheme={onToggleTheme} active="Community" title="Loading discussion"><div className="travel-panel travel-empty"><span className="travel-loading-dot" /><h3>Loading discussion...</h3></div></TravelShell>;
  if (!question || !bundle) return <TravelShell onNavigate={onNavigate} theme={theme} onToggleTheme={onToggleTheme} active="Community" title="Discussion"><div className="travel-panel travel-empty"><h3>Discussion not found</h3><button className="travel-button" onClick={() => onNavigate('community')}>Back to community</button></div></TravelShell>;

  return <TravelShell onNavigate={onNavigate} theme={theme} onToggleTheme={onToggleTheme} active="Community" title="Community discussion">
    <button className="travel-back-link" onClick={() => onNavigate('community')}><ArrowLeft size={14} /> Back to Community</button>
    <div className="travel-grid forum-detail-layout"><main><article className="travel-panel travel-panel-pad forum-question-detail"><div className="forum-detail-top"><span className="forum-category">{question.category}</span><span className="forum-detail-destination">📍 {question.destination}</span></div><h2>{question.title}</h2><Person user={author} /><p className="forum-question-content">{question.content}</p>{question.tripContext && <div className="travel-alert"><span>🧳</span><span>Trip context shared by the author: {question.tripContext.travelers} travelers · ৳{Number(question.tripContext.budget || 0).toLocaleString()} · {question.tripContext.dates}</span></div>}<div className="forum-tags">{question.tags.map((tag) => <span key={tag}>#{tag}</span>)}</div><div className="forum-detail-actions"><button className={`forum-action-button ${question.liked ? 'selected' : ''}`} onClick={likeQuestion}><Heart size={15} fill={question.liked ? 'currentColor' : 'none'} /> {question.likes} likes</button><button className={`forum-action-button ${question.bookmarked ? 'selected' : ''}`} onClick={toggleBookmark}><Bookmark size={15} fill={question.bookmarked ? 'currentColor' : 'none'} /> {question.bookmarked ? 'Saved' : 'Save'}</button><button className={`forum-action-button ${question.followed ? 'selected' : ''}`} onClick={toggleFollow}><Bell size={15} fill={question.followed ? 'currentColor' : 'none'} /> {question.followed ? 'Following' : 'Follow Discussion'}</button></div></article>
