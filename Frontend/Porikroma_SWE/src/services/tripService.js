...
export const tripService = {
  ...
  getDiscoverTrips: (travelType) => useMock ? wait(BANGLADESH_TRIPS) : apiClient.get('/api/v1/trips', { discover: true, travel_type: travelType }).then((response) => response.data?.items || []),
  ...
  getJoinRequests: (tripId) => useMock ? wait([]) : apiClient.get(`/api/v1/trips/${tripId}/join-requests`).then((response) => response.data || []),
  createJoinRequest: (tripId, message = 'Request to join group') => useMock ? wait({ id: `req-${Date.now()}`, status: 'pending' }) : apiClient.post(`/api/v1/trips/${tripId}/join-requests`, { message }).then((response) => response.data),
  updateJoinRequest: (tripId, requestId, status) => useMock ? wait({ id: requestId, status }) : apiClient.patch(`/api/v1/trips/${tripId}/join-requests/${requestId}`, { status }).then((response) => response.data)
};
